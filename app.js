const state = { puzzles: [], puzzle: null, date: null, direction: 'across', selected: null, entries: {}, cells: {}, answers: {}, elapsed: 0, running: true, timerId: null, pendingAction: null, completionNoticeShown: false };
...
async function loadPuzzle(date) {
  stopTimer(); state.date = date; state.completionNoticeShown = false;
...
function nextIncompleteEntry(entries, startIndex, endIndex = entries.length) { for (let index = startIndex; index < endIndex; index++) { const entry = entries[index]; const emptyIndex = firstEmptyIndex(entry); if (emptyIndex !== null) return { entry, index: emptyIndex }; } return null; }
function isEntryComplete(entry) { return entry.cells.every((key) => state.cells[key]?.dataset.value); }
function nextEntry() {
  ...
  if (next) return moveToEntry(next.entry, next.index, state.direction);
  ...
  const earlier = currentIndex > 0 ? nextIncompleteEntry(currentEntries, 0, currentIndex) : null;
  if (earlier) return moveToEntry(earlier.entry, earlier.index, state.direction);
}
...
function updateProgress() { const complete = Object.values(state.cells).every((cell) => cell.dataset.value); if (complete && !state.completionNoticeShown) { state.completionNoticeShown = true; finish(); } }
...
function checkAnswers() { Object.entries(state.cells).forEach(([key, cell]) => { if (!cell.dataset.value) return; cell.classList.toggle('correct', cell.dataset.value === state.answers[key]); cell.classList.toggle('wrong', cell.dataset.value !== state.answers[key]); }); renderSelection(); $('#statusMessage').textContent = 'Las respuestas se han marcado'; }
...
function showConfirmation(title, text, action) { state.pendingAction = action; $('#modalIcon').hidden = true; $('#modalTitle').textContent = title; $('#modalText').textContent = text; $('#modalAction').textContent = 'Confirmar'; $('#modalAction').hidden = false; $('#modalCancel').textContent = 'Cancelar'; $('#modalCancel').hidden = false; $('#modal').classList.remove('hidden'); }
function showOutcome(title, text, icon = '') { state.pendingAction = null; $('#modalIcon').textContent = icon; $('#modalIcon').hidden = !icon; $('#modalTitle').textContent = title; $('#modalText').textContent = text; $('#modalAction').hidden = true; $('#modalCancel').textContent = 'Seguir jugando'; $('#modalCancel').hidden = false; $('#modal').classList.remove('hidden'); }
...
function countWrongEntries() { return orderedEntries().filter(({ entry }) => entry.cells.some((key) => state.cells[key]?.dataset.value !== state.answers[key])).length; }
function finish() { const wrongEntries = countWrongEntries(); if (wrongEntries) { showOutcome('Hay respuestas por revisar', `Tienes ${wrongEntries} ${wrongEntries === 1 ? 'palabra mal formada' : 'palabras mal formadas'}. Puedes seguir jugando.`); return; } stopTimer(); showOutcome('¡Crucigrama terminado!', `Has completado el crucigrama en ${formatTime(state.elapsed)}.`, '🏆'); }
