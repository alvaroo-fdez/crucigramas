...
function updateProgress() { const total = Object.keys(state.answers).length; const filled = Object.values(state.cells).filter((cell) => cell.dataset.value).length; if (filled === total && !state.completionNoticeShown) { state.completionNoticeShown = true; finish(); } }
